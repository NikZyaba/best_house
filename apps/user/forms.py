...
class CustomUserCreationForm(UserCreationForm):
    email = forms.EmailField(required=True, max_length=50, widget=forms.EmailInput(attrs={"class": "input-register form-control", "placeholder": "Your email"}))
    ...
    password1 = forms.CharField(required=True, max_length=50, widget=forms.EmailInput(attrs={"class": "input-register form-control", "placeholder": "Your password"}))
    password2 = forms.CharField(required=True, max_length=50, widget=forms.EmailInput(attrs={"class": "input-register form-control", "placeholder": "Confirm Your password"}))
    marketing_consent1 = forms.BooleanField(required=False, label="I agree to recieve commercial, promotional and marketing communications.", widget=forms.CheckboxInput(attrs={"class": "checkbox-input-register"}))
    marketing_consent2 = forms.BooleanField(required=False, label="I agree to recieve commercial, promotional and marketing communications.", widget=forms.CheckboxInput(attrs={"class": "checkbox-input-register"}))
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists:
            raise forms.ValidationError("Email is used already")
        return email
...
class CustomUserLoginForm(AuthenticationForm):
    ...
    password = forms.CharField(label="Password", widget=forms.TextInput(attrs={"autofocus": True, "class": "input-register form-control", "placeholder": "Your password"}))
    ...
    def clean(self):
        ...
        if email and password:
            ...
            if self.user_cache is None:
                ...
                raise forms.ValidationError("Account is unactive") 
